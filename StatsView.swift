        ...
        NavigationView {
            ScrollView {
                VStack(spacing: 20) {
                    ...
                    // Lista de categorías
                    VStack(spacing: 16) {
                        ForEach(viewModel.expensesByCategory.sorted(by: { $0.value > $1.value }), id: \.key) { category, amount in
                            HStack {
                                ...
                                VStack(alignment: .leading) {
                                    Text(category.rawValue)
                                        .font(.system(.body))
                                    Text("\(viewModel.expenses.filter { $0.category == category }.count) movimientos")
                                        .font(.caption)
                                        .foregroundColor(.gray)
                                }
                                ...
                                VStack(alignment: .trailing) {
                                    ...
                                    GeometryReader { geometry in
                                        ZStack(alignment: .leading) {
                                            ...
                                            Rectangle()
                                                .fill(category.color)
                                                .frame(width: geometry.size.width * CGFloat(amount/viewModel.totalExpenses), height: 4)
                                        }
                                    }
